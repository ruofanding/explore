...
class DeepLRMF(torch.nn.Module):
    ...
    def forward(self, user_id, movie_id, rating):
        ...
        x = torch.concat((user_embedding.repeat([1, 1 + self.neg_num, 1]), movie_embedding), axis = -1)
    ...
    def recall(self, user_id, topk):
        ...
        x = torch.concat((uembed_stack.view(-1, 8), movie_embedding.view(-1, 8)), axis = -1)
        for i, fc in enumerate(self.fc_stack):
            x = fc(x)
            if i < len(self.fc_stack) - 1:
                x = F.relu(x)
        x.view(batch_dim, movie_cnt)
        logits = x

...
class LRMF(torch.nn.Module):
    ...
    def __init__(self, emb_dim, user_num, movie_num, init_factor):
        super().__init__()
    ...
    def forward(self, user_id, movie_id):
        ...
        user_embedding = self.user_embedding(user_id)
        movie_embedding = self.movie_embedding(all_movie_id)

        user_embedding = F.normalize(user_embedding, dim=2)
        movie_embedding = F.normalize(movie_embedding, dim=2)

        dot_product = torch.sum(user_embedding * movie_embedding, dim=2)
